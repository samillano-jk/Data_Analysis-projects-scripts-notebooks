...
def calculate_demographic_data(print_data=True):
    # Read data from file
    df = pd.read_csv("/workspace/boilerplate-demographic-data-analyzer/demographic_data_analyzer.py")
    ...
    num_min_workers = df.loc[(df["hours-per-week"]== min_hour_per_week), "hours-per-week"].count()

    rich_percentage = np.round(df.loc[(df["hours-per-week"]== min_hour_per_week) & (df["salary"]==">50K"), "hours-per-week"].count() / num_min_workers * 100, 1)
    ...
    new_country = country_count.pivot_table(index="native-country", columns="salary", values="count")
